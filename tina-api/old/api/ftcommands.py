...
ftcommands = Namespace('/ft/', description='Api for Fasttext')




@ftcommands.route('/ping')
...
@ftcommands.response(400, 'failed.')
@ftcommands.route('/load', methods=['POST'])
class load(Resource):
    @ftcommands.response(201, 'loaded : ok')
    @ftcommands.expect(loadmodel) #name, version, supervised, quantized
    def post(self):
...
@ftcommands.response(400, 'failed.')
@ftcommands.route('/predict', methods=['POST'])
class predict(Resource):
    @ftcommands.response(201, 'prediction : ok')
    @ftcommands.expect(prediction) #modelname, (version), text, nbofresults
    def post(self):
...
@ftcommands.response(400, 'failed.')
@ftcommands.route('/train', methods=['POST'])
class train(Resource):
    @ftcommands.response(201, 'ok')
    @ftcommands.expect(training) #name, version, supervised, quantized
    def post(self):
        post_data = request.get_json()
...
@ftcommands.response(400, 'failed.')
@ftcommands.route('/getstate', methods=['POST'])
class getstate(Resource):
    @ftcommands.response(201, 'loaded : ok')
    @ftcommands.expect(getstatemodel) #name, version, supervised, quantized
    def post(self):
        post_data = request.get_json()
